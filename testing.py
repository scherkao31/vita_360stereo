...
def main():
    ...
    for inx in tqdm(range(len(test_up_img))):
        ...
        if args.real:
            ...
            imgU = np.reshape(imgU, [1, 4, imgU.shape[1], imgU.shape[2]])
            imgD = np.reshape(imgD, [1, 4, imgD.shape[1], imgD.shape[2]])
        print("ICI :", imgD.shape)
        if imgD.shape[2] == 500: #ADDED
            imgU = np.pad(imgU, ((0, 0), (0, 0), (6, 6), (0, 0)), mode='constant')
            imgD = np.pad(imgD, ((0, 0), (0, 0), (6, 6), (0, 0)), mode='constant')
        print("Post pad :", imgD.shape)
        # Wide padding -----------------------------
        LR_pad = 32
